...
def calculator(bot, update):
    ...
    is_match = re.match('^(\d+)(\/|-|\+|\*)(\d+)=$', user_text)

    if not is_match:
        update.message.reply_text("Вы ввели неправильные данные. Правильный пример: 3/5=")
        return

    first_addend = is_match.group(1)
    operator = is_match.group(2)
    second_addend = is_match.group(3)
    try:
